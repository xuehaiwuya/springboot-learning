...
public class CustomMetadataSource implements FilterInvocationSecurityMetadataSource {
    ...
    public Collection<ConfigAttribute> getAttributes(Object o) {
        ...
        List<Menu> allMenu = JsonUtil.string2Obj(redisUtil.get(RedisConst.ALL_MENU), List.class, Menu.class);
        if (CollectionUtils.isEmpty(allMenu)) {
            allMenu = menuService.getAllMenu();
            redisUtil.setEx(RedisConst.ALL_MENU, JsonUtil.obj2String(allMenu), RedisConst.DEFAULT_REDIS_SESSION_TIMEOUT);
        }
        for (Menu menu : allMenu) {
            //如果不需要访问权限则默认登录访问
            if (menu.getRequireAuth() == 0) {
                return SecurityConfig.createList(RedisConst.ROLE_LOGIN);
            }
